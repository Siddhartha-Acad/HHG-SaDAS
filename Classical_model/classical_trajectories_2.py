"""
File: classical_trajectories_2.py
Project: HHG-SaDAS
Code Description:
...
ax1 = fig1.add_subplot(121); ax11 = fig1.add_subplot(122)
ax2 = fig2.add_subplot(121); ax22 = fig2.add_subplot(122)
...
ax2.plot(phi_0_1st_qtr / np.pi, KE_low_norm, 'o-', color='#7F3FBF', label=rf'K$_{{max}}$ = {np.round(max(KE_low_norm), 3)}U$_p$', markersize=4)
ax2.axvline(KE_max_low_phi_0, lw=1.5, linestyle='dashed', color='blue', alpha=0.7, label=rf'φ$_0$={np.round(KE_max_low_phi_0, 3)}π')

KE_max_high_phi_0 = phi_0_3rd_qtr[np.argmax(KE_high_norm)] / np.pi
ax22.plot(phi_0_3rd_qtr / np.pi, KE_high_norm, 'o-', color='#BF3F7F', label=rf'K$_{{max}}$ = {np.round(max(KE_high_norm), 3)}U$_p$', markersize=4)
ax22.axvline(KE_max_high_phi_0, lw=1.5, linestyle='dashed', color='red', alpha=0.7, label=rf'φ$_0$={np.round(KE_max_high_phi_0, 3)}π')
