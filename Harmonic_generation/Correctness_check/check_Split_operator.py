...
psi_file = 'Ar@C60_States__l=1_w0=0.324_sigma=1.077_rc=6.690_p=2_nos=10_N=200_rmax=200_Lmap=20_Anim.xlsx'
psi_file = this_dir / 'states_and_Smatrix' / psi_file
psi_data = pd.read_excel(psi_file, header=None, skiprows=1).to_numpy().T

S_matrix_file = 'Ar@C60_Smatrix__m=1_w0=0.324_sigma=1.077_rc=6.690_p=2_lmax=20_kmax=50_N=200_rmax=200_Lmap=20_dt=0.1_Anim.xlsx'
S_matrix_file = this_dir / 'states_and_Smatrix' / S_matrix_file
S_matrix_data = pd.read_excel(S_matrix_file, header=None, skiprows=1).to_numpy().T
S_matrix = np.array([[complex(*map(float, elem.split(','))) for elem in column] for column in S_matrix_data]).reshape(L, N-1, N-1)
...
R_m, _ = np.meshgrid(A_r, theta)                  # Initial wavefunction ~ determined by (n, l).
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~: G(l) = S(l) * g(l) :~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
psi_0_recon = np.zeros((len(theta), len(r)), dtype=np.complex128)     # Reconstructed ψ0(r, θ) = A(r) • P_l(cosθ)
psi_1 = np.zeros((len(theta), len(r)), dtype=np.complex128)           # ψ1(r, θ) = exp{-iH0(dt)/2} • ψ0(r, θ)
psi_2 = np.zeros((len(theta), len(r)), dtype=np.complex128)           # ψ2(r, θ) = exp{-iV(r, θ, t+dt/2)(dt)/2} • ψ1(r, θ)
psi_evolved = np.zeros((len(theta), len(r)), dtype=np.complex128)     # ψ(r, θ, t+dt) = exp{-iH0(dt)/2} • ψ2(r, θ)
cos_theta = np.cos(theta); len_theta = len(theta)
Y_lm_cos_theta_j = np.array([[Y_lm(l_ind+m, m, cos_theta[j]) for j in range(len(theta))] for l_ind in range(L)])
...
for j in range(len(theta)):
    ...
    psi_2[j] = np.exp(-1j * V_int(r, theta[j], t[0] + dt / 2) * dt) * psi_1[j]
...
ax4.plot(np.diff(theta), 'o-', color='m', label='dθ')

ax9.plot(r, np.abs(psi_0[50]) ** 2, 'o-', markersize=10, label=r'|A(θ[50], t=0)|$^2$')
ax9.plot(r, np.abs(psi_evolved[50]) ** 2, 'o--', color='m', label=r'|A(θ[50], t=dt)|$^2$')
